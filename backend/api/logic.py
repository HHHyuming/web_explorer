...
def get_cascade_path(root_file_path):
    ...
    while data_stack:
        ...
        if master_node.next or master_node.data:
            try:
                # if os.path.isfile(master_node.data['path']):
                #     print(master_node.data['path'])
                #
                #     continue
                ...
                if isinstance(master_node.next.data, list):
                    if bool(master_node.next):
                        ...
                        if len(master_node.data['children']) == 0:
                            master_node.data.pop('children')
            except Exception:
                continue
